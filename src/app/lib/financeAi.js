...
export async function analyzeFinanceImage(buffer, mimeType = "image/jpeg", extraText = "") {
  ...
  const prompt = `อ่านภาพหลักฐานทางการเงินภาษาไทย/อังกฤษ เช่นสลิปธนาคาร ใบเสร็จ หรือภาพหน้าจอธุรกรรม แล้วคืน JSON object เท่านั้น ห้าม markdown โดยมี keys: direction (income|expense), amount (number), transaction_date (ISO-8601 ถ้าไม่ทราบให้ null), category, description, counterparty, bank_name, reference_no, project_name, confidence (0-1). อ่านยอดเงินจริง วันที่/เวลา ธนาคารต้นทาง/ปลายทาง และเลขอ้างอิงจากภาพให้ละเอียด. บัญชีบริษัทอาจแสดงชื่อ ธานีแอดเวอร์ไทซิ่ง / ศิวนนท์ หรือเลขบัญชีปิดท้าย 4034, 1403, 8829. ถ้าบริษัทเราเป็นผู้รับให้ direction=income; ถ้าบริษัทเราเป็นผู้โอน/ผู้จ่ายให้ direction=expense. ห้ามเดาทิศทางจากข้อความประกอบอย่างเดียว. หมวดแนะนำ: รายได้งานป้าย, เงินมัดจำ, ค่าวัสดุ, ค่าแรง, ค่าน้ำมัน/เดินทาง, ค่าเครื่องมือ, ค่าใช้จ่ายสำนักงาน, ภาษี/ค่าธรรมเนียม, อื่น ๆ. ข้อความประกอบจาก LINE/ผู้ใช้: ${extraText || "ไม่มี"}`;
