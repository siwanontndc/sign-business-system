...
function own(s){const n=normalizeName(s);return (n.includes('ธานี')&&(n.includes('แอดเวอร์')||n.includes('แอดเวอ')||n.includes('advertis')))||n.includes('ศิวนนท์')||n.includes('ศค้วนนท์')||n.includes('ศุภฐิติ')||n.includes('ศุภฐติ');}
...
export function extractSlipParties(text=''){
 ...
 const from=[],to=[];let mode='',count=0;
 for(const line of lines){
  const label=line.replace(/^[^ก-๙A-Za-z]*/,'');
  if(sender.test(label)){mode='from';count=0;from.push(label.replace(sender,'').trim());continue;}
  if(recipient.test(label)){mode='to';count=0;to.push(label.replace(recipient,'').trim());continue;}
  if(stop.test(label)){mode='';continue;}
  if(!mode)continue;
  if(++count>5){mode='';continue;}
  if(account.test(label)||/^(?:กรุงไทย|กสิกรไทย|ไทยพาณิชย์|กรุงเทพ|กรุงศรี|ออมสิน|ทหารไทยธนชาต|krungthai|kasikorn|scb|ttb)/i.test(label))continue;
  if(/\d{1,2}\s*[:\/.\-]\s*\d{2}/.test(label))continue;
  (mode==='from'?from:to).push(label);
 }
 return{from:from.join(' ').trim(),to:to.join(' ').trim()};
}
export function inferSlipDirection(text='',options={}){
 const {from,to}=extractSlipParties(text);
 const fromOwn=own(from),toOwn=own(to);
 if(fromOwn&&toOwn)return'';
 if(fromOwn)return'expense';
 if(toOwn)return'income';
 return'';
}
