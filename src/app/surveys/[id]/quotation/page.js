...
export default function SurveyQuotationPage(){
  ...
  async function create(){
    try{
      ...
      if(!s.customer_id && !String(s.customer_name||'').trim()) throw new Error('กรุณากรอกชื่อลูกค้าก่อนสร้างใบเสนอราคา');
      ...
      const {data:q,error:qe}=await supabase.from('quotations').insert({
        ...
        project_name:s.project_name||`งานจากการสำรวจ ${s.survey_no}`,
        quotation_date:now.toISOString().slice(0,10),valid_days:30,subtotal:0,discount:0,vat_percent:0,vat_amount:0,grand_total:0,status:'draft',note:surveyNote
      }).select().single(); if(qe)throw qe;
