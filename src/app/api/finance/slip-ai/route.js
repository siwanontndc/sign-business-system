...
export async function POST(req){
  try{
    if(!process.env.OPENAI_API_KEY)return Response.json({ok:false,error:'AI_NOT_CONFIGURED'},{status:503});
    ...
    if(!r.ok)return Response.json({ok:false,error:'AI_REQUEST_FAILED',detail:data?.error?.message||''},{status:502});
    const text=(data.output||[]).flatMap(x=>x.content||[]).filter(x=>x.type==='output_text').map(x=>x.text||'').join('\n')||data.output_text||'';
    const j=safeJson(text);
    if(!j)return Response.json({ok:false,error:'AI_INVALID_JSON'},{status:502});
    ...
    return Response.json({ok:false,error:'AI_SERVER_ERROR'},{status:500});
  }
}
