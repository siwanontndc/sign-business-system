...
const btn={padding:"12px 10px",borderRadius:10,fontWeight:800,fontSize:16,minHeight:48,whiteSpace:"nowrap"};
...
export default function LineFinancePage(){
  ...
  return <main style={{minHeight:"100vh",background:"#f4f6f8",padding:"14px 10px 90px",color:"#111827"}}><div style={{maxWidth:1180,margin:"auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:14}}>
      <div><h1 style={{margin:"0 0 4px",fontSize:28}}>💬 บัญชีจาก LINE</h1><div style={{color:"#6b7280"}}>อ่านสลิปบนอุปกรณ์ ตรวจสอบก่อนบันทึกเงินจริง</div></div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><a href="/finance">การเงิน</a><a href="/finance/reports">📊 รายงาน</a><a href="/finance/categories">🏷️ หมวดหมู่</a></div>
    </div>
    ...
    {loading?<p>กำลังโหลด...</p>:visible.length===0?<div style={{background:"white",padding:28,borderRadius:12,textAlign:"center"}}>ไม่มีรายการ</div>:visible.map(r=>{
      ...
      return <section key={r.id} style={{background:"white",border:`1px solid ${!known?"#cbd5e1":isIncome?"#bfdbfe":"#fecdd3"}`,borderRadius:16,padding:14,marginBottom:14,boxShadow:"0 2px 8px rgba(15,23,42,.05)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}><div><strong>{r.message_type==="text"?"ข้อความ LINE":"สลิป / หลักฐานจาก LINE"}</strong><div style={{fontSize:12,color:"#6b7280",marginTop:3}}>{new Date(r.event_at||r.created_at).toLocaleString("th-TH",{timeZone:"Asia/Bangkok"})} • {r.status}</div></div><div style={{fontWeight:800,color:accent,background:soft,padding:"8px 12px",borderRadius:999}}>{!known?"⚠ ยังไม่ทราบประเภท":isIncome?"↑ รายรับ":"↓ รายจ่าย"}</div></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,320px),1fr))",gap:16,marginTop:12}}>
          ...
          <div>{r.status==="pending"?<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,220px),1fr))",gap:10,marginTop:12}}>
              <label style={{fontWeight:700}}>ประเภท<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:5}}><button type="button" onClick={()=>change(r.id,"direction","income")} style={{...btn,border:d.direction==="income"?"2px solid #0b6cff":"1px solid #d1d5db",background:d.direction==="income"?"#0b6cff":"white",color:d.direction==="income"?"white":"#111827"}}>↑ รายรับ</button><button type="button" onClick={()=>change(r.id,"direction","expense")} style={{...btn,border:d.direction==="expense"?"2px solid #e11d48":"1px solid #d1d5db",background:d.direction==="expense"?"#e11d48":"white",color:d.direction==="expense"?"white":"#111827"}}>↓ รายจ่าย</button></div></label>
              <label style={{fontWeight:700}}>จำนวนเงิน<input style={{...input,marginTop:5,fontWeight:900,fontSize:22,color:accent}} type="number" step="0.01" value={d.amount??""} onChange={e=>change(r.id,"amount",e.target.value)}/></label>
              <label>หมวดหมู่<select style={{...input,marginTop:5}} value={d.category||""} disabled={!known} onChange={e=>change(r.id,"category",e.target.value)}><option value="">เลือกประเภทก่อน</option>{cats.map(c=><option key={c} value={c}>{c}</option>)}</select></label>
              <label>วันที่ทำรายการ<input style={{...input,marginTop:5}} type="date" value={d.transaction_date||""} onChange={e=>change(r.id,"transaction_date",e.target.value)}/></label>
              <label>งาน / Job<input style={{...input,marginTop:5}} value={d.project_name||""} onChange={e=>change(r.id,"project_name",e.target.value)}/></label>
              <label>คู่ค้า / ผู้โอน<input style={{...input,marginTop:5}} value={d.counterparty||""} onChange={e=>change(r.id,"counterparty",e.target.value)}/></label>
              <label>ธนาคาร<input style={{...input,marginTop:5}} value={d.bank_name||""} onChange={e=>change(r.id,"bank_name",e.target.value)}/></label>
              <label>เลขอ้างอิง<input style={{...input,marginTop:5}} value={d.reference_no||""} onChange={e=>change(r.id,"reference_no",e.target.value)}/></label>
              <label style={{gridColumn:"1/-1"}}>รายละเอียด<input style={{...input,marginTop:5}} value={d.description||""} onChange={e=>change(r.id,"description",e.target.value)}/></label>
            </div>
            <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}><button disabled={busy||!known||!goodAmount(d.amount)||!d.transaction_date} onClick={()=>review(r,"approve")} style={{...btn,border:0,background:known?"#16a34a":"#94a3b8",color:"white",flex:"1 1 180px"}}>ยืนยันและบันทึก</button><button disabled={busy} onClick={()=>review(r,"reject")} style={{...btn,border:"1px solid #d1d5db",background:"white",flex:"1 1 120px"}}>ปฏิเสธ</button></div>
